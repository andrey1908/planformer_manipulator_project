...
from segmentation import segment_red_boxes_hsv, segment_blue_boxes_hsv, \
    segment_goal_hsv, segment_stop_line_hsv, segmnet_nn, segment_table_markers_hsv
...
def segment_scene_colorful(image, view=""):
    if not use_nn:
        ...
        (tm_refined_mask, tm_mask), tm_polygons = segment_table_markers_hsv(hsv, view=view)
    else:
    ...
    segmentation[:, :, 1] = np.maximum(segmentation[:, :, 1], tm_refined_mask)
